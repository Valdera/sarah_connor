...
class Environment(object):
    ...
    def update_env(self, direction, energy_ai, month):
        ...
        '''
        # --- GETTING THE REWARD ---
        # Computing the energy spent by the server's cooling system where there is no AI
        ...
        self.current_number_users += np.random.randint(-self.min_number_users,  self.max_number_users)
        ...
        # Computing the Delta temperature caused by AI
        if (direction == -1):
            delta_temperature_ai = -energy_ai
        elif (direction == 1):
            delta_temperature_ai = energy_ai
        # Updating the new server temperature when there is AI
        self.temperature_ai += delta_intrinsic_temperature + delta_temperature_ai
        # Updating the new server temperature when there is no AI
    ...
    def observe(self):
        '''
        Gives us at any time the current state, the last reward and whether the game is over
        '''
        scaled_temperature_ai = (self.temperature_ai - self.min_temperature) / (self.max_temperature - self.min_temperature)
        scaled_number_users = (self.current_number_users - self.min_number_users ) / (self.max_number_users - self.min_number_users)
        scaled_rate_data = (self.current_rate_data - self.min_rate_data) / (self.max_rate_data - self.min_rate_data)
